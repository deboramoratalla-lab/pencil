...
const BASE = [
  ...
  { id:"tt-top",  platform:"TikTok",   name:"TopView",         w:1080,h:1920,fam:"portrait", nativeCta:true },
  { id:"ig-story",platform:"Instagram",name:"Story",           w:1080,h:1920,fam:"portrait", nativeCta:true },
  { id:"ig-reel", platform:"Instagram",name:"Reel",            w:1080,h:1920,fam:"portrait", nativeCta:true },
  { id:"sc-story",platform:"Snapchat", name:"Story",           w:1080,h:1920,fam:"portrait", nativeCta:true },
  { id:"yt-short",platform:"YouTube",  name:"Shorts",          w:1080,h:1920,fam:"portrait", nativeCta:true },
];
const ADDABLE = [
  { id:"tt-feed", platform:"TikTok",   name:"In-Feed",  w:1080,h:1920,fam:"portrait", nativeCta:true },
...
// Prototype template geometry. Footer insets are illustrative, not platform specifications.
function measureCopy(text,px,width){
  const rows=[];let line="",wide=false;
  for(const word of String(text||"").split(/\s+/)){
    if(textW(word,px)>width)wide=true;
    const next=line?`${line} ${word}`:word;
    if(line&&textW(next,px)>width){rows.push(line);line=word;}else line=next;
  }
  rows.push(line);return {rows,height:rows.length*px*1.12,wide};
}
function composition(f,values,adaptation=false){
  const W=f.dw,H=f.dh,portrait=f.fam==="portrait",pad=W*.07;
  const local=typeof adaptation==="object"?adaptation:{headline:adaptation};
  const headlineAdjusted=local.headline===true||(typeof local.headline==="number"&&local.headline<1);
  const expanded=headlineAdjusted||(local.subhead||1)<1;
  const platformSafeArea=portrait;
  const strictPlatformSafeArea=portrait&&["TikTok","YouTube"].includes(f.platform);
  const platformUiInset=portrait?W*.10:0;
  const safeBottom=H*(strictPlatformSafeArea?.79:portrait?.91:.95);
  const fineOffset=values.fineprint?(portrait?.06:f.fam==="landscape"?.08:.12):0;
  const top=H*((portrait?(expanded?.42:strictPlatformSafeArea?.53:.59):f.fam==="landscape"?(expanded?.20:.39):expanded?.28:.64)-fineOffset);
  const width=portrait?W-pad-platformUiInset:f.fam==="landscape"?W*.59:W-2*pad;
  const flowRoles=["headline","subhead"];
  const bottomRoles=[...(!f.nativeCta?["cta"]:[]),...(values.fineprint?["fineprint"]:[])];
  const roles=[...flowRoles,...bottomRoles];
  const base={headline:W*(portrait?.12:f.fam==="landscape"?.068:.082),subhead:W*(f.fam==="landscape"?.034:.045),cta:W*.036,fineprint:W*.028};
  const minimum={headline:W*(f.fam==="landscape"?.048:.08),subhead:W*.028,cta:W*.028,fineprint:W*.028};
  const gap=W*.022;
  const layout=scale=>{
    const items={};
    const makeItem=role=>{
      const roleScale=role==="headline"?(typeof local.headline==="number"?local.headline:(local.headline ? 0.70 : 1)):role==="subhead"?(local.subhead||1):role==="cta"?(local.cta||1):1;
      const px=Math.max(minimum[role],base[role]*scale*roleScale),inset=role==="cta"?W*.09:0;
      const measured=measureCopy(values[role],px,width-inset);
      const height=measured.height+(role==="cta"?W*.035+2:0);
      return {...measured,px,height,width:width-inset};
    };
    let y=top;
    for(const role of flowRoles){
      const item=makeItem(role);items[role]={...item,y};y+=item.height+gap;
    }
    const flowBottom=y-gap;
    let bottomCursor=safeBottom;
    for(const role of [...bottomRoles].reverse()){
      const item=makeItem(role);bottomCursor-=item.height;items[role]={...item,y:bottomCursor};bottomCursor-=gap;
    }
    const bottomStart=bottomRoles.length?items[bottomRoles[0]].y:safeBottom;
    const bottom=Math.max(...roles.map(role=>items[role].y+items[role].height));
    return {items,bottom,flowBottom,bottomStart};
  };
  const fits=l=>l.flowBottom<=l.bottomStart-gap+.1&&l.bottom<=safeBottom+.1&&roles.every(r=>!l.items[r].wide);
  // Propagation preserves the shared type style. A format only diverges after
  // the user accepts a local composition change.
  const original=layout(1),result=original,scale=1;
  const conflict=!fits(result);
  const states={};
  roles.forEach(r=>{const item=result.items[r];const locallyFitted=(r==="subhead"&&local.subhead<1)||(r==="cta"&&local.cta<1);const collides=flowRoles.includes(r)&&item.y+item.height>result.bottomStart-gap+.1;const breaksButton=r==="cta"&&item.rows.length>1;states[r]=item.wide||collides||breaksButton||item.y<0||item.y+item.height>safeBottom+.1?"conflict":(scale<1||locallyFitted)?"auto":"fits";});
  if(conflict&&result.items.headline.height>base.headline*2.24)states.headline="conflict";
  return {...result,states,conflict,scale,pad,width,safeBottom,platformSafeArea,strictPlatformSafeArea,expanded,top};
}
...
function Scene({fmt,onPick,selected,anchor,expanded=false}){
...
export default function PencilCanvas(){
 ...
 const [walk,setWalk]=useState(-1),[walkPaused,setWalkPaused]=useState(false);const walkSaved=useRef(null),walkStates=useRef([]);
  ...
  const loadSnapshot=s=>{setFormats(s.formats);setContent(s.content);setHasFine(s.hasFine);setOv(s.ov);setUnrev(s.unrev);setApproved(s.approved||{});setFmtAdapted(s.fmtAdapted);setFmtSubAdapted(s.fmtSubAdapted||{});setFmtCtaAdapted(s.fmtCtaAdapted||{});setAdapted(false);setFocus(null);setPin(false);};
  ...
  const carries=useCallback((f,el)=> el==="cta"?!f.nativeCta : el==="fineprint"?FAM[f.fam].fine : true,[]);
  const layoutFor=useCallback(f=>composition(
    f,
    Object.fromEntries(["headline","subhead","cta","fineprint"].filter(e=>e!=="fineprint"||(hasFine&&carries(f,e))).map(e=>[e,valIn(e,f.id)])),
    {headline:fmtAdapted[f.id]||1,subhead:fmtSubAdapted[f.id]||1,cta:fmtCtaAdapted[f.id]||1}
  ),[valIn,fmtAdapted,fmtSubAdapted,fmtCtaAdapted,hasFine,carries]);
  const overflows=useCallback((el,f)=>carries(f,el)&&layoutFor(f).states[el]==="conflict",[layoutFor,carries]);
  const elementScale=(el,f)=>el==="cta"?(fmtCtaAdapted[f.id]||1):el==="subhead"?(fmtSubAdapted[f.id]||1):(fmtAdapted[f.id]||1);
  ...
  const walkLabels=["Watch the shared headline across every format.","The headline is selected once and linked everywhere.","The headline is edited once and updates live across the five 9:16 formats.","The shared style remains the same. Three compositions fit; two platform safe areas conflict.","AI adapted only the two affected layouts. Review both results before approving.","Approved changes are green. Explore manually to change scope, unlink elements, add formats or inspect consistency."];
  const captureWalk=()=>({...snapshot(),view,sel,subset,scopeMode,anchor,focus,pin,adapted,adaptedIds,adaptedCount,audit,historyLog,history:structuredClone(history.current)});
  const restoreWalk=q=>{loadSnapshot(q);setView(q.view);setSel(q.sel);setSubset(q.subset);setScopeMode(q.scopeMode);setAnchor(q.anchor);setFocus(q.focus);setPin(q.pin);setAdapted(q.adapted);setAdaptedIds(q.adaptedIds);setAdaptedCount(q.adaptedCount);setAudit(q.audit);setHistoryLog(q.historyLog);history.current=q.history;setNotice(null);setTyping(false);};
  const runWalkthrough=()=>{
    if(walk>=0)return;walkSaved.current=captureWalk();walkStates.current=[];
    clearTimeout(typingTimer.current);clearTimeout(noticeTimer.current);
    setFormats(BASE);setContent({logo:"L'ORÉAL",headline:"Mirror-Shine Hair",subhead:"Powered by Argan Oil",cta:"SHOP NOW"});
    setHasFine(false);setOv({});setUnrev({});setApproved({});setFmtAdapted({});setFmtSubAdapted({});setFmtCtaAdapted({});
    setSel(null);setSubset([]);setScopeMode("all");setAdapted(false);setAdaptedIds([]);setFocus(null);setPin(false);setDemo(false);setTyping(false);
    setAudit(false);setFmtMenu(false);setPanelMenu(false);setZoomMenu(false);setNotice(null);setDemo(true);setWalkPaused(false);setWalk(0);requestAnimationFrame(()=>fit());
  };
  const nextWalk=()=>{
    walkStates.current[walk]=captureWalk();const ids=placed.filter(f=>f.fam==="portrait").map(f=>f.id);
    if(walk===0){showFormats([placed[0].id],"headline");setSubset([]);setScopeMode("all");}
    if(walk===1){showFormats(ids,"headline");setSubset(ids);setScopeMode("vertical");}
    if(walk===2){
      const copy="Discover shiny hair powered by argan oil";
      let index=0;
      setTyping(true);
      const typeNext=()=>{
        index=Math.min(copy.length,index+1);
        const value=copy.slice(0,index);
        setOv(o=>({...o,headline:{...(o.headline||{}),...Object.fromEntries(ids.map(id=>[id,value]))}}));
        if(index<copy.length)typingTimer.current=setTimeout(typeNext,58);
        else {setTyping(false);typingTimer.current=setTimeout(()=>setWalk(3),700);}
      };
      typeNext();
      return;
    }
  ...
  const backWalk=()=>{if(walk>0&&walkStates.current[walk-1]){clearTimeout(typingTimer.current);setTyping(false);restoreWalk(walkStates.current[walk-1]);setWalkPaused(true);setWalk(w=>w-1);}};
  const exitWalk=()=>{clearTimeout(typingTimer.current);clearTimeout(noticeTimer.current);clearTimeout(walkTimer.current);if(walkSaved.current)restoreWalk(walkSaved.current);setWalkPaused(false);setWalk(-1);};
  const replayWalk=()=>{
    clearTimeout(walkTimer.current);walkStates.current=[];
    setFormats(BASE);setContent({logo:"L'ORÉAL",headline:"Mirror-Shine Hair",subhead:"Powered by Argan Oil",cta:"SHOP NOW"});
    setHasFine(false);setOv({});setUnrev({});setApproved({});setFmtAdapted({});setFmtSubAdapted({});setFmtCtaAdapted({});
    setSel(null);setSubset([]);setScopeMode("all");setAdapted(false);setAdaptedIds([]);setFocus(null);setPin(false);setTyping(false);
    setAudit(false);setNotice(null);setDemo(true);setWalkPaused(false);setWalk(0);requestAnimationFrame(()=>fit());
  };
  useEffect(()=>{
    clearTimeout(walkTimer.current);
    if(walk<0||walk>=5||walkPaused)return;
    const advance=()=>walk<3?nextWalk():walk===3?adapt():confirmAdaptations();
    const delay=walk===0?2800:walk<3?4000:walk===4?7600:5200;
    walkTimer.current=setTimeout(advance,delay);
  ...
  const broken=sel?targets.filter(f=>overflows(sel,f)&&!approved[`${f.id}:${sel}`]):[];
  const canAutoAdapt=f=>{
    if(!sel)return false;
    const preview={
      headline:fmtAdapted[f.id]||1,
      subhead:fmtSubAdapted[f.id]||1,
      cta:fmtCtaAdapted[f.id]||1
    };
    if(sel==="headline")preview.headline=.70;
    if(sel==="subhead")preview.subhead=.70;
    if(sel==="cta")preview.cta=.70;
    const values=Object.fromEntries(["headline","subhead","cta","fineprint"]
      .filter(e=>e!=="fineprint"||(hasFine&&carries(f,e)))
      .map(e=>[e,valIn(e,f.id)]));
    return composition(f,values,preview).states[sel]!=="conflict";
  };
  const improvableBroken=broken.filter(canAutoAdapt);
  const manualBroken=broken.filter(f=>!canAutoAdapt(f));
  ...
  const layoutIssueIds=useMemo(()=>placed.filter(f=>els.some(el=>
    overflows(el,f)&&!approved[`${f.id}:${el}`]
  )).map(f=>f.id),[placed,els,overflows,approved]);
  const layoutIssueCount=layoutIssueIds.length;
  const pendingFormatIds=useMemo(()=>[...new Set([
    ...layoutIssueIds,
    ...Object.keys(unrev).map(key=>key.split(":")[0])
  ])],[layoutIssueIds,unrev]);
  const pendingCount=pendingFormatIds.length;
  useEffect(()=>{
  ...
  const edit=v=>{
    checkpoint("Edit "+LABEL[sel]);setAdapted(false);const ids=targets.map(f=>f.id);
    setApproved(a=>Object.fromEntries(Object.entries(a).filter(([key])=>!ids.some(id=>key===`${id}:${sel}`))));
    setUnrev(a=>Object.fromEntries(Object.entries(a).filter(([key])=>!ids.some(id=>key===`${id}:${sel}`))));
    if(sel==="headline")setFmtAdapted(a=>Object.fromEntries(Object.entries(a).filter(([id])=>!ids.includes(id))));
    if(sel==="subhead")setFmtSubAdapted(a=>Object.fromEntries(Object.entries(a).filter(([id])=>!ids.includes(id))));
    if(sel==="cta")setFmtCtaAdapted(a=>Object.fromEntries(Object.entries(a).filter(([id])=>!ids.includes(id))));
    setTyping(true);clearTimeout(typingTimer.current);typingTimer.current=setTimeout(()=>setTyping(false),650);
    if(subset.length)setOv(o=>({...o,[sel]:{...(o[sel]||{}),...Object.fromEntries(ids.map(id=>[id,v]))}}));else setContent(c=>({...c,[sel]:v}));
  };
  ...
  const adapt=()=>{
    if(!improvableBroken.length)return;if(walk===3)walkStates.current[3]=captureWalk();
    checkpoint("Adapt local composition");const ids=improvableBroken.map(f=>f.id);
    setAdapted(true);setAdaptedCount(ids.length);setAdaptedIds(ids);
    if(sel==="headline")setFmtAdapted(m=>({...m,...Object.fromEntries(ids.map(id=>[id,.70]))}));
    if(sel==="subhead")setFmtSubAdapted(m=>({...m,...Object.fromEntries(ids.map(id=>[id,.70]))}));
    if(sel==="cta")setFmtCtaAdapted(m=>({...m,...Object.fromEntries(ids.map(id=>[id,.70]))}));
    setApproved(a=>Object.fromEntries(Object.entries(a).filter(([k])=>!ids.includes(k.split(":")[0]))));
    setUnrev(u=>({...u,...Object.fromEntries(ids.map(id=>[id+":"+sel,true]))}));showFormats(ids);if(walk===3)setWalk(4);
  };
  ...
  const retryFormat=fid=>{
    ...
    checkpoint();
    if(sel==="headline")setFmtAdapted(m=>({...m,[fid]:.70}));
    if(sel==="subhead")setFmtSubAdapted(m=>({...m,[fid]:.70}));
    if(sel==="cta")setFmtCtaAdapted(m=>({...m,[fid]:.70}));
    setUnrev(u=>({...u,[`${fid}:${sel}`]:true}));
    pick(sel,fid,false);goTo(f);
  };
  const retryAll=()=>{
    checkpoint();
  ...
  const approveElement=el=>{
    const approvable=placed.filter(f=>unrev[`${f.id}:${el}`]&&(el==="fineprint"||!overflows(el,f)));
    setApproved(a=>{const n={...a};approvable.forEach(f=>{n[`${f.id}:${el}`]=true;});return n;});
    setUnrev(u=>{const n={...u};approvable.forEach(f=>delete n[`${f.id}:${el}`]);return n;});
  };
  const confirmAdaptations=()=>{
    if(adaptedIds.some(fid=>{const f=placed.find(p=>p.id===fid);return f&&overflows(sel||"headline",f);}))return;
    if(walk===4)walkStates.current[4]=captureWalk();checkpoint("Approve adaptations");
    setApproved(a=>{const n={...a};adaptedIds.forEach(fid=>{n[`${fid}:${sel}`]=true;});return n;});
    setUnrev(u=>{const n={...u};adaptedIds.forEach(fid=>delete n[`${fid}:${sel}`]);return n;});
    setAdapted(false);if(walk===4)setWalk(5);
  ...
  const addFine=()=>{
    ...
    setHasFine(true);setContent(c=>({...c,fineprint:"Terms apply."}));
    ...
    setSel("fineprint");setSubset(touched);setScopeMode("selection");setAnchor(touched[0]||null);
    setReviewEditing(false);setFocus(touched);setPin(true);setCopyOpen(false);
  };
  const addFormat=f=>{
    ...
    if(addAuto){
      const s=Math.sqrt(AREA/(f.w*f.h));
      const candidate={...f,dw:Math.round(f.w*s),dh:Math.round(f.h*s)};
      const values={headline:content.headline,subhead:content.subhead,cta:content.cta,
        ...(hasFine&&carries(f,"fineprint")?{fineprint:content.fineprint}:{} )};
      let local={headline:1,subhead:1,cta:1};
      ["headline","subhead","cta"].filter(el=>carries(f,el)).forEach(el=>{
        if(composition(candidate,values,local).states[el]!=="conflict")return;
        const trial={...local,[el]:.70};
        if(composition(candidate,values,trial).states[el]!=="conflict")local=trial;
      });
      if(local.headline<1)setFmtAdapted(m=>({...m,[f.id]:local.headline}));
      if(local.subhead<1)setFmtSubAdapted(m=>({...m,[f.id]:local.subhead}));
      if(local.cta<1)setFmtCtaAdapted(m=>({...m,[f.id]:local.cta}));
    }
    announce([f.id]);
  };
  const removeFormat=fid=>{
    ...
    setApproved(u=>{const n={...u};Object.keys(n).forEach(k=>{if(k.startsWith(fid+":"))delete n[k];});return n;});
    setFmtAdapted(m=>{const n={...m};delete n[fid];return n;});
    setFmtSubAdapted(m=>{const n={...m};delete n[fid];return n;});
    ...
    setAdaptedIds(ids=>ids.filter(id=>id!==fid));
    setSubset(ids=>ids.filter(id=>id!==fid));
    setFocus(ids=>ids?ids.filter(id=>id!==fid):ids);
  ...
  const setPresetZoom=k=>{
    const el=wrap.current;if(!el)return;
    const fitsWidth=totalW*k<=el.clientWidth-72;
    const fitsHeight=maxH*k<=el.clientHeight-96;
    setView({
      k,
      x:fitsWidth?(el.clientWidth-totalW*k)/2:36,
      y:fitsHeight?(el.clientHeight-maxH*k)/2+10:48
    });
  };
  useEffect(()=>{fit();},[fit]);
  ...
  return (
    <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",background:"#fff",
      ...
      <style>{`
@keyframes guidePulse {50% {box-shadow:0 0 0 5px #9b85ff66;}}
.walk-target {outline:2px solid #9272ff!important;outline-offset:3px;animation:guidePulse 1.6s ease-in-out infinite;}
.side-rail {overflow-y:auto;overflow-x:hidden;scrollbar-width:none;overscroll-behavior:contain;}
.side-rail::-webkit-scrollbar {display:none;}
.side-rail > * {flex-shrink:0;}
@media(prefers-reduced-motion:reduce){.walk-target{animation:none;}}
      ...
      {walk>=0&&<div role="region" aria-label="Guided walkthrough" style={{position:"absolute",bottom:18,left:72,right:"calc(clamp(190px, 34vw, 308px) + 70px)",zIndex:60,background:"#fff",border:"1px solid #ddd",borderRadius:12,padding:"12px 16px",boxShadow:"0 4px 24px #0002",display:"flex",gap:12,alignItems:"center"}}>
<span style={{fontSize:11,whiteSpace:"nowrap"}}>{walk+1} / 6</span>
<div aria-live="polite" style={{fontSize:12,flex:1,lineHeight:1.5}}>{walkLabels[walk]}</div>
<button style={ghost} disabled={walk===0} onClick={backWalk}>Back</button>
{walk<5&&<button style={walkPaused?primary:ghost} onClick={()=>setWalkPaused(p=>!p)}>{walkPaused?"Play":"Pause"}</button>}
{walk===5&&<button style={primary} onClick={replayWalk}>Replay</button>}
<button style={walk===5?ghost:{...bare,color:"#6b6960",padding:"6px 4px"}} onClick={exitWalk}>{walk===5?"Explore manually":"Exit"}</button></div>}
      {/* ---------------- top bar ---------------- */}
      <div className="native-header" style={{height:TOP,flexShrink:0,display:"flex",alignItems:"center",gap:12,padding:"0 12px 0 8px",
        ...
        <div style={{position:"relative"}}>
          ...
          {zoomMenu&&<div style={{position:"absolute",top:32,left:0,background:"#fff",padding:5,border:"1px solid #e6e3dd",borderRadius:8,minWidth:135}}>
            ...
            {[.25,.5,1,1.5,2].map(k=><button key={k} onClick={()=>{setPresetZoom(k);setZoomMenu(false);}} style={{...bare,display:"block",width:"100%",textAlign:"left",padding:8}}>{k*100}%</button>)}
          </div>}
        </div>
        <div style={{flex:1}}/>
        <button onClick={runWalkthrough} disabled={walk>=0} title="Play walkthrough" style={{...ghost,fontSize:11.5,padding:"6px 9px",whiteSpace:"nowrap"}}>Play walkthrough</button>
      ...
      <div style={{flex:1,display:"flex",minHeight:0}}>
        {/* ---------------- left rail ---------------- */}
        <div className="side-rail" style={{width:RAIL,flexShrink:0,minHeight:0,borderRight:"1px solid #ececea",display:"flex",
          ...
          <div style={{position:"absolute",transformOrigin:"0 0",transform:`translate(${view.x}px,${view.y}px) scale(${view.k})`}}>
            ...
            {placed.map(f=>{
              ...
              const elState = (!sel||isAbs) ? null
                ...
                : "linked";
              const isBrk=sel&&!isAbs&&overflows(sel,f)&&!approved[`${f.id}:${sel}`];
              ...
              const hasLayoutIssue=els.some(el=>overflows(el,f)&&!approved[`${f.id}:${el}`]);
              ...
              return (
                <div key={f.id} style={{position:"absolute",left:f.x,top:f.y,
                  ...
                  <Ad fmt={f} els={els} valIn={valIn} carries={carries} sel={sel} isAbs={isAbs}
                    ...
                    demoRing={demo&&!sel?"headline":null} walkStep={walk}
                    showRing={true}
        ...
        {/* ---------------- right panel ---------------- */}
        <div style={{width:"clamp(190px,34vw,308px)",flexShrink:0,minWidth:0,borderLeft:"1px solid #ececea",overflowY:"auto",zIndex:20}}
          ...
          {fmtMenu?(
            <>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 15px",
                ...
                <div>
                  ...
                  <div style={{fontSize:10.5,color:"#93908a",marginTop:2}}>{formats.length} currently on canvas · duplicates allowed</div>
                </div>
              ...
              <div style={{padding:"12px 15px",borderBottom:"1px solid #f2f0eb"}}>
                ...
                <div style={{fontSize:11,color:"#93908a",marginTop:5,lineHeight:1.4}}>
                  {addAuto
                    ? "Pre-fits shared text to each new format. Every result remains available for review."
                    : "Adds the shared layers unchanged. Adjust the new format manually with Pencil’s native tools."}
                </div>
              </div>
                      ...
                      {open&&items.map(f=>{
                        ...
                        return (
                          <div key={f.id} style={{padding:"7px 15px 7px 34px",background:instances.length?"#faf9f7":"transparent"}}>
                            ...
                            <button aria-label={`Add ${f.platform} ${f.name}`} onClick={()=>addFormat(f)} style={{...ghost,padding:"4px 9px",display:"flex",fontSize:11.5}}>
                              {instances.length?"Duplicate":"Add"}
                            </button>
                            </div>
            ...
            <>
              ...
              <div style={{padding:"14px 15px"}}>
                ...
                {sel!=="fineprint"&&<div style={{display:"flex",gap:5,marginBottom:10,flexWrap:"wrap"}} aria-label="Propagation scope">
                ...
                {sel!=="fineprint"&&anchor&&placed.find(f=>f.id===anchor)&&(
                ...
                {sel&&targets.some(f=>layoutFor(f).states[sel]==="auto")&&<div style={{fontSize:11,color:"#777",marginTop:10}}>{targets.filter(f=>layoutFor(f).states[sel]==="auto").length} auto-fitted · copy unchanged</div>}
                ...
                {(sel==="headline"||sel==="subhead"||sel==="cta")&&broken.length>0&&(
                  <div style={{marginTop:13,display:"flex",alignItems:"center",gap:8,fontSize:12.5}}>
                    ...
                    <span style={{color:"#3d3b35"}}>{broken.length} formats need attention</span>
                    <div style={{display:"flex",gap:5,marginLeft:"auto"}}>
                      ...
                      <button onMouseEnter={()=>preview(broken.map(f=>f.id))} onMouseLeave={endPreview}
                        onClick={()=>showFormats(broken.map(f=>f.id))}
                ...
                {sel==="fineprint"&&(
                  <div style={{marginTop:13,fontSize:12.5,color:"#6b6960",lineHeight:1.5}}>
                    Fine print has been added as one shared layer to {targets.length} supported formats.
                    {absent.length>0&&` It is not added to ${absent.length} formats whose native template does not include a fine-print layer.`}
                    {reviewCount>0&&(
                      <div style={{marginTop:11,paddingTop:10,borderTop:"1px solid #ece8e1"}}>
                        <div style={{color:"#3d3b35",marginBottom:8}}>Edit or review this set once, then approve the placement across all {targets.length} formats together.</div>
                ...
                {(sel==="headline"||sel==="subhead"||sel==="cta")&&broken.length>0&&!typing&&(
                  <div style={{marginTop:13,background:"#f7f5ff",border:"1px solid #e9e4ff",borderRadius:10,padding:11}}>
                    ...
                    <div style={{fontSize:12,color:"#4a4840",lineHeight:1.45}}>
                      {improvableBroken.length>0
                        ? `The shared ${LABEL[sel].toLowerCase()} conflicts in ${broken.length} formats. I can adapt ${improvableBroken.length} without changing the copy.`
                        : `The shared ${LABEL[sel].toLowerCase()} cannot fit legibly in ${broken.length} formats. Shorten the copy or edit those layouts manually.`}
                      {improvableBroken.length>0&&manualBroken.length>0&&
                        ` ${manualBroken.length} will still need manual editing.`}
                    </div>
                    <div style={{display:"flex",gap:7,marginTop:10}}>
                      {improvableBroken.length>0&&<button className={walk===3?"walk-target":""} onClick={adapt} style={primary} title="Apply recommended layout adaptation"><AiIcon size={13}/> Improve {improvableBroken.length}</button>}
                      {improvableBroken.length===0&&<button onClick={()=>document.querySelector(input)?.focus()} style={primary}>Edit copy</button>}
                      <button onClick={()=>{setSel(null);setSubset([]);setScopeMode("all");setReviewEditing(false);clearSpot();}} style={ghost}>Review later</button>
                    </div>
                  </div>
                )}
                {adapted&&adaptedIds.length>0&&adaptedIds.every(fid=>{const f=placed.find(p=>p.id===fid);return f&&!overflows(sel,f);})&&(
                  <div style={{marginTop:12,background:"#f5f1ff",border:"1px solid #d9ccff",
                    ...
                    <div style={{display:"flex",gap:7,marginTop:9,alignItems:"center"}}>
                      <button className={walk===4?"walk-target":""} onClick={confirmAdaptations}
        ...
        <div className="side-rail" style={{width:RAIL,flexShrink:0,minHeight:0,borderLeft:"1px solid #ececea",display:"flex",
          flexDirection:"column",alignItems:"center",paddingTop:10,gap:4,zIndex:20}}>
          {RIGHT_NAV.map((ico)=>{
...
function Ad({fmt,els,valIn,carries,sel,isAbs,isBrk,isDiv,adapted,subAdapted=1,ctaAdapted=1,elState,isAnchor,demoRing,walkStep=-1,showRing=true,onPick}){
  ...
  const ring=k=>{
    ...
    const st = elState==="review" ? "review" : elState==="approved" ? "approved" : isDiv ? "unlinked" : "linked";
  ...
  const geometry=composition(
    fmt,
    Object.fromEntries(["headline","subhead","cta","fineprint"].filter(k=>k!=="fineprint"||(els.includes(k)&&carries(fmt,k))).map(k=>[k,valIn(k,fmt.id)])),
    {headline:adapted,subhead:subAdapted,cta:ctaAdapted}
  );
  const platformUiName=fmt.platform==="TikTok"?"TikTok":fmt.platform==="YouTube"?"YouTube Shorts":fmt.platform==="Snapchat"?"Snapchat":fmt.name==="Reel"?"Instagram Reels":"Instagram Stories";
  const headlineConflicts=geometry.states.headline==="conflict";
  const showPlatformGuide=fmt.fam==="portrait"&&sel==="headline"&&((walkStep>=2&&walkStep<=4)||headlineConflicts);
  const guideColour=headlineConflicts?"#ffa928":"rgba(255,255,255,.62)";
  const guideFill=headlineConflicts?"#ffb00018":"rgba(255,255,255,.055)";
  ...
  return (
    <div style={{width:fmt.dw,height:fmt.dh,borderRadius:5,overflow:"hidden",position:"relative",
      ...
      <Scene fmt={fmt} onPick={onPick} selected={sel} anchor={isAnchor} expanded={geometry.expanded}/>
      {PHOTO && (
      ...
      {Object.entries(geometry.items).map(([role,item])=><div key={role} className={role==="headline"&&(walkStep===0||walkStep===1||(walkStep===2&&fmt.fam==="portrait"))?"walk-target":""} data-copy={role} data-state={geometry.states[role]} onClick={e=>onPick(role,e)} style={{
position:"absolute",left:geometry.pad,top:item.y,width:role==="cta"?Math.max(...item.rows.map(t=>textW(t,item.px))):item.width,boxSizing:"content-box",
color:role==="headline"?"white":"#fff3c7",fontFamily:"Georgia, serif",fontSize:item.px,lineHeight:1.12,
background:role==="cta"?"#b77a12":undefined,padding:role==="cta"?W*.0175+"px "+W*.045+"px":0,
border:role==="cta"?"1px solid #f4d477":undefined,cursor:"pointer",...ring(role)}}>
{item.rows.map((row,i)=><span key={i} style={{display:"block",whiteSpace:"nowrap"}}>{row||"\u00a0"}</span>)}</div>)}
{showPlatformGuide&&<>
  <div title="Illustrative placement safe area" style={{position:"absolute",pointerEvents:"none",top:geometry.safeBottom,left:0,right:0,bottom:0,
    borderTop:`1px dashed ${guideColour}`,background:`repeating-linear-gradient(135deg,${guideFill} 0 4px,transparent 4px 8px)`,
    color:headlineConflicts?"#ffde92":"rgba(255,255,255,.82)",fontSize:W*.034,padding:W*.016,letterSpacing:".01em"}}>
    {platformUiName} UI safe area
  </div>
  <div aria-hidden="true" style={{position:"absolute",pointerEvents:"none",right:0,top:"17%",bottom:"21%",width:"10%",
    borderLeft:`1px dashed ${guideColour}`,background:`repeating-linear-gradient(135deg,${guideFill} 0 4px,transparent 4px 8px)`,
  }}/>
</>}
      {fmt.fam==="portrait"&&(
